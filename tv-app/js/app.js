...
(function() {
    ...
    App._setupNav = function() {
        ...
        if (logoDiv) {
            ...
            if (!versionEl) {
                ...
                versionEl.id = 'nav-version';
                versionEl.style.cssText = 'font-size:20px;font-weight:300;color:rgba(255,255,255,0.85);letter-spacing:0.5px;margin-left:6px;align-self:flex-end;padding-bottom:6px';
                logoDiv.appendChild(versionEl);
            }
    ...
    // With dynamic script loading, DOMContentLoaded may have already fired
    function startApp() {
        App.init();
        ...
        setTimeout(function() {
            ...
            if (logoImg && !document.getElementById('nav-version')) {
                ...
                v.id = 'nav-version';
                v.style.cssText = 'display:inline-block;vertical-align:middle;font-size:16px;font-weight:300;color:rgba(255,255,255,0.85);letter-spacing:0.5px;margin-left:8px';
                v.textContent = 'v' + appVersion;
                logoImg.parentNode.insertBefore(v, logoImg.nextSibling);
            }
        }, 3000);
    }
