        ...
        _handleYearsNav: function(key) {
            ...
            switch (key) {
                ...
                case App.Config.KEYS.OK:
                    // Get target year
                    var targetYear = null;
                    if (idx > 0 && this._allYears[idx - 1]) {
                        targetYear = this._allYears[idx - 1].year;
                    }
                    var sameYear = (targetYear === this._selectedYear);
                    if (!sameYear) {
                    ...
                    if (this._movieElements.length > 0) {
                        this._activePanel = 'grid';
                        this._clearYearFocus();
                        this._updateGridFocus(sameYear ? this._gridFocusIndex : 0);
                    }
                    break;
            }
        },
