        ...
        if (storageConfig.mode === 'local' && fsSync.existsSync(storageConfig.localPath)) {
            ...
            try {
                ...
                const list = await withFTPClient(FTP_CONFIG, async (client) => {
                    // Archivos en la raíz
                    const rootFiles = await client.list(seriesPath);
                    const allFiles = [];
                    rootFiles.filter(item => !item.isDirectory && extensions.some(ext => item.name.toLowerCase().endsWith(ext)))
                        .forEach(item => allFiles.push({ name: item.name, size: `${(item.size / (1024 * 1024)).toFixed(1)} MB` }));
                    // Subcarpetas
                    const subdirs = rootFiles.filter(item => item.isDirectory);
                    for (const sub of subdirs) {
                        try {
                            const subList = await client.list(`${seriesPath}/${sub.name}`);
                            subList.filter(item => !item.isDirectory && extensions.some(ext => item.name.toLowerCase().endsWith(ext)))
                                .forEach(item => allFiles.push({ name: item.name, size: `${(item.size / (1024 * 1024)).toFixed(1)} MB`, subfolder: sub.name }));
                        } catch (e) {
                            // subcarpeta no legible
                        }
                    }
