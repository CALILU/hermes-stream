 ...
 * Prueba puertos UPnP comunes y extrae el friendlyName
 */
async function addManualDevice(ip) {
    const ports = [49152, 8008, 8060, 1400, 52235, 9197, 7676, 1900];
    const xmlPaths = [
    ...
    for (const port of ports) {
        for (const xmlPath of xmlPaths) {
            ...
            if (result) {
                ...
                state.devices.set(result.url, device);
                saveManualDevices();
                console.log(`📺 DLNA: Dispositivo manual añadido: ${result.name} (${ip}:${port})`);
                return device;
            }
        }
    }
