...
function pause() {
    return new Promise((resolve, reject) => {
        if (state.lgtvClient) {
            state.lgtvClient.request('ssap://media.controls/pause', (err) => {
                if (err) return reject(err);
                state.playbackStatus = { ...state.playbackStatus, state: 'paused' };
            ...
            reject(new Error('No hay reproduccion activa'));
        }
    });
}
...
function resume() {
    return new Promise((resolve, reject) => {
        if (state.lgtvClient) {
            state.lgtvClient.request('ssap://media.controls/play', (err) => {
                if (err) return reject(err);
                state.playbackStatus = { ...state.playbackStatus, state: 'playing' };
                resolve({ success: true });
            });
