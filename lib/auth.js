...
const JWT_REFRESH_EXPIRY = process.env.JWT_REFRESH_EXPIRY || '3650d';
...
function authMiddleware(usersDB) {
    return (req, res, next) => {
        ...
        if (ALLOW_LAN_AUTH && isLocalIP(clientIP)) {
            // Try to identify user by TV serial or model header
            const tvSerial = req.headers['x-tv-serial'];
            const tvModel = req.headers['x-tv-model'];
            if (usersDB) {
                let tvUser = null;
                if (tvSerial && usersDB.findTVBySerial) {
                    tvUser = usersDB.findTVBySerial(tvSerial);
                }
                if (!tvUser && tvModel && usersDB.findTVByModel) {
                    tvUser = usersDB.findTVByModel(tvModel);
                }
                if (tvUser) {
                    req.user = { id: tvUser.user_id, username: tvUser.username, role: tvUser.role, isLocal: true };
                    return next();
                }
            }
            // Fallback: generic local admin (PC, browser, unregistered TV)
