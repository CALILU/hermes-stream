...
function normalizeCacheToAPI(cached) {
    ...
    return {
        ...
        cast: normalizeCast(cached.cast || []),
...
/**
 * Normalizar cast photos (convertir TMDB URLs legacy a proxy URLs).
 */
function normalizeCast(cast) {
    return cast.map(actor => ({
        ...actor,
        photo: actor.photo ? ensureFullPosterURL(actor.photo, 'h632') : null
    }));
}
