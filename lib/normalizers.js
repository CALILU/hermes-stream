...
function normalizeCacheToAPI(cached) {
    ...
    return {
        ...
        poster: ensureFullPosterURL(cached.poster || cached.poster_path),
        backdrop: ensureFullPosterURL(cached.backdrop || cached.backdrop_path),
...
function normalizeRecommendations(recommendations) {
    return recommendations.map(rec => ({
        ...
        poster: ensureFullPosterURL(rec.poster || rec.poster_path),
    ...
    if (details.recommendations?.results) {
        result.recommendations = details.recommendations.results
            ...
            .map(r => ({
                ...
                title: r.title,
                overview: r.overview || '',
                poster_path: r.poster_path ? `${imageBase}${r.poster_path}` : null,
    ...
    if (details.credits?.cast) {
        result.cast = details.credits.cast
            ...
            .map(actor => ({
                ...
                photo: actor.profile_path ? `${imageBase}${actor.profile_path}` : null
            }));
    }
